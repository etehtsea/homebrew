...
module Homebrew extend self
  def fetch
    ...
    bucket.each do |f|
      ...
      previous_sha256 = f.instance_variable_get(:@sha256)

      puts "MD5:  #{the_tarball.md5}"
      puts "SHA1: #{the_tarball.sha1}"
      puts "SHA256: #{the_tarball.sha256}"
      ...
      unless previous_sha256.nil? or previous_sha256.empty? or  the_tarball.sha256 == previous_sha256
        opoo "Formula reports different SHA256: #{previous_sha256}"
      end
    end
  end
end
