module DownloadStrategy
  # raised in DownloadStrategy::Curl.fetch
  ...
  class StrictSubversion < Subversion
    def svn
      exe = super
  ...
  class UnsafeSubversion < Subversion
    def fetch_repo(target, url, revision = nil, ignore_externals = false)
      # Use "svn up" when the repository already exists locally.
      # This saves on bandwidth and will have a similar effect to verifying the
      # cache as it will make any changes to get the right revision.
  ...
  def self.detect(url)
    ...
    when %r[^git://]; Git
    when %r[^hg://];  Mercurial
    when %r[^svn://]; Subversion
    when %r[^svn\+http://]; Subversion
    when %r[^fossil://]; Fossil
    # Some well-known source hosts
    when %r[^https?://github\.com/.+\.git$]; Git
    when %r[^https?://(.+?\.)?googlecode\.com/hg]; Mercurial
    when %r[^https?://(.+?\.)?googlecode\.com/svn]; Subversion
    when %r[^https?://(.+?\.)?sourceforge\.net/svnroot/]; Subversion
    when %r[^http://svn.apache.org/repos/]; Subversion
    when %r[^http://www.apache.org/dyn/closer.cgi]; CurlApacheMirror
