  ...
  module Doctor
    ...
    module Check
      class << self
        ...
        def stray_dylibs
          unbrewed_dylibs = Dir['/usr/local/lib/*.dylib'].select do |f|
            File.file? f and not File.symlink? f
          end
        ...
        def stray_pcs
          unbrewed_pcs = Dir['/usr/local/lib/pkgconfig/*.pc'].select do |f|
            File.file? f and not File.symlink? f
          end
        ...
        def stray_las
          unbrewed_las = Dir['/usr/local/lib/*.la'].select do |f|
            File.file? f and not File.symlink? f
          end
        ...
        def gcc_42
          ...
          elsif MacOS.gcc_42_build_version < Homebrew.recommended_gcc_42
            <<-EOS.undent
        ...
        def xcode_exists
          if MacOS.xcode_version == 'unknown'
            ...
            elsif MacOS.gcc_40_build_version < Homebrew.recommended_gcc_40
              <<-EOS.undent
        ...
        def writable_install_location
          if Homebrew.cellar.exist? and not Homebrew.cellar.writable?
            "Cannot write to #{Homebrew.cellar}"
          end
          unless Homebrew.prefix.writable? or Homebrew.prefix.to_s == '/usr/local'
            "Cannot write to #{Homebrew.prefix}"
          end
        end

        def access_share_locale
          __check_subdir_access 'share/locale'
        end

        def access_share_man
          __check_subdir_access 'share/man'
        end

        def access_pkgconfig
          ...
          path_folders.each do |p| case p
            when '/usr/bin'
              ...
              unless $seen_prefix_bin
                ...
                if conflicts.size > 0
                  out = <<-EOS.undent
                    /usr/bin occurs before #{Homebrew.prefix}/bin
                    This means that system-provided programs will be used instead of those
                    provided by Homebrew. The following tools exist at both paths:

                        #{conflicts * "\n                "}

                    Consider ammending your PATH so that #{Homebrew.prefix}/bin
        ...
        def config_scripts
          ...
          path_folders.each do |p|
            dir_list = ['/usr/bin', '/usr/sbin', '/usr/X11/bin', '/usr/X11R6/bin',
                        "#{Homebrew.prefix}/bin", "#{Homebrew.prefix}/sbin",
                        "/opt/X11/bin"]
            next if dir_list.include? p

            if real_cellar
              next if p =~ %r[^(#{real_cellar.to_s}|#{Homebrew.cellar.to_s})]
            end
        ...
        def symlinked_cellar
          if Homebrew.cellar.symlink?
            <<-EOS.undent
              ...
              (A) Have Cellar be a real directory inside of your Homebrew.prefix
        ...
        def MACOSX_DEPLOYMENT_TARGET
          ...
          if target_var and target_var != Macos.version.to_s then <<-EOS.undent
        ...
        def tmpdir
          tmpdir = ENV['TMPDIR']
          unless tmpdir.nil? or File.directory? tmpdir
          ...
          Homebrew.repository.cd do
            unless `git status -s -- Library/Homebrew/ 2>/dev/null`.chomp.empty? then <<-EOS.undent
              ...
              Unless you know what you are doing, you should run:
                cd #{Homebrew.repository} && git reset --hard
              EOS
            end
          end
        end
        ...
        def unlinked_but_not_keg_only
          unlinked = Homebrew.cellar.children.reject do |rack|
            if not rack.directory?
              true
            elsif not (Homebrew.linked_kegs/rack.basename).directory?
        ...
        def cellar_exists
          FileUtils.mkdir_p Homebrew.cellar unless File.exist? Homebrew.cellar
        rescue
          <<-EOS.undent
